...
PROJ_ROOT = osp.normpath(osp.join(_CUR_DIR, "../.."))
...
import torch
from lib.egl_renderer import CppEGLRenderer
...
class Renderer(object):
    ...
    phong/color/textured during runtime."""
    ...
    def __init__(
        ...
        gpu_id = _GPU_ID if gpu_id is None else gpu_id
        self.r = CppEGLRenderer.CppEGLRenderer(width, height, gpu_id)
        self.r.init()
        ...
        self.color_tex = gu.Texture(GL_TEXTURE_2D, 1, GL_RGBA32F, W, H)
        self.color_tex_2 = gu.Texture(GL_TEXTURE_2D, 1, GL_RGBA32F, W, H)
        self.color_tex_3 = gu.Texture(GL_TEXTURE_2D, 1, GL_RGBA32F, W, H)
        self.render_buffer = gu.Renderbuffer(GL_DEPTH_COMPONENT32F, W, H)

        self.color_tex_21 = gu.Texture(GL_TEXTURE_2D, 1, GL_RGBA32F, W, H)
        self.color_tex_22 = gu.Texture(GL_TEXTURE_2D, 1, GL_RGBA32F, W, H)
        self.render_buffer_2 = gu.Renderbuffer(GL_DEPTH_COMPONENT32F, W, H)
        self._fbo = gu.Framebuffer(
            {
                GL_COLOR_ATTACHMENT0: self.color_tex,
                GL_COLOR_ATTACHMENT1: self.color_tex_2,
                GL_COLOR_ATTACHMENT2: self.color_tex_3,  # for normal mapping
                GL_DEPTH_ATTACHMENT: self.render_buffer,
            }
        )

        self._fbo_depth = gu.Framebuffer(
            {
                GL_COLOR_ATTACHMENT0: self.color_tex_21,
                GL_COLOR_ATTACHMENT1: self.color_tex_22,
                GL_DEPTH_ATTACHMENT: self.render_buffer_2,
            }
        )
        ...
        textureless_texture_shader = gu.Shader(
            "depth_shader_textureless_texture_4.vs",
            "depth_shader_textureless_texture_4.frag",
        )
    ...
    def render(
        ...
        if depth_tensor is not None:
            self.r.map_tensor(
                int(self.color_tex_2.id),
                int(self.width),
                int(self.height),
                depth_tensor.data_ptr(),
            )
            depth_tensor.data = torch.flip(depth_tensor, (0,))
        if normal_tensor is not None:
            self.r.map_tensor(
                int(self.color_tex_3.id),
                int(self.width),
                int(self.height),
                normal_tensor.data_ptr(),
            )
            normal_tensor.data = torch.flip(normal_tensor, (0,))
        # glNamedFramebufferReadBuffer(self._fbo.id, GL_COLOR_ATTACHMENT0)
        # if to_255:
        #     bgr_flipped = np.frombuffer(
        #         glReadPixels(0, 0, self.width, self.height, GL_BGR, GL_UNSIGNED_BYTE), dtype=np.uint8).reshape(self.height, self.width, 3)
        # else:
        #     bgr_flipped = np.frombuffer(
        #         glReadPixels(0, 0, self.width, self.height, GL_BGR, GL_FLOAT), dtype=np.float32).reshape(self.height, self.width, 3)
    ...
    def render_many(
        ...
        for i in range(len(obj_ids)):
            ...
            if depth_tensor is not None:
                self.r.map_tensor(
                    int(self.color_tex_22.id),
                    int(self.width),
                    int(self.height),
                    depth_tensor.data_ptr(),
                )
                cur_depth = torch.flip(depth_tensor, (0,))[:, :, 0]
                mask = (cur_depth > 0).to(torch.uint8)
                ys_xs = mask.nonzero(as_tuple=False)
                ys, xs = ys_xs[:, 0], ys_xs[:, 1]

                bb_tl = [xs.min().item(), ys.min().item()]
                bb_br = [xs.max().item(), ys.max().item()]
                obj_bb_xyxy = [bb_tl[0], bb_tl[1], bb_br[0], bb_br[1]]
                if with_mask:
                    masks.append(mask)
...
def test_render_many():
    ...
    depth_tensor = torch.cuda.FloatTensor(height, width, 4).detach()
    ...
    for i in tqdm(range(runs)):
        ...
        bbs, masks = renderer.render_many(
            obj_ids,
            Rs,
            ts,
        ...
        if False:  # show
            ...
            masks = [_m.cpu().numpy() for _m in masks]
...
def test_render_ycbv():
    ycbv_path = "datasets/BOP_DATASETS/ycbv"
    model_dir = osp.join(ycbv_path, "models")
    file_path = osp.join(ycbv_path, 'data/ycbv.yaml')
    ...
    ZNEAR = 0.25
    ZFAR = 6.0
    object_id_file = osp.join(PROJ_ROOT, file_path)
    ...
    image_tensor = torch.cuda.FloatTensor(height, width, 4).detach()
    depth_tensor = torch.cuda.FloatTensor(height, width, 4).detach()
    normal_tensor = torch.cuda.FloatTensor(height, width, 4).detach()
    ...
    if True:
        ...
        # total runs: 63000, time: 41.30841883737594s, speed: 1525.1128407509386 fps
    ...
    if True:
        for i in range(1):
            for obj_id, cls_name in enumerate(classes):
                ...
                plt.imshow(normal)
                plt.axis("off")
                plt.title("{} normal".format(cls_name))
...
def test_render_many_ycbv():
    ...
    """
    t sample 0.0031424823733977973s, t_render: 0.0056793316104449335s t_total: 0.00882181398384273s, fps: 113.35537133649761
    """
...
if __name__ == "__main__":
    ...
    if True:
        ...
        dt = time.perf_counter() - start
        print("total runs: {}, time: {}s, speed: {} fps".format(run, dt, run / dt))
        # total runs: 45000, time: 32.42778921127319s, speed: 1387.6986712481837 fps
        # much faster than cpu version
